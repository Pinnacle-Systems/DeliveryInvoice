...
async function getNextDocId(branchId, shortCode, startTime, endTime, saveType, docId, isUpdate) {
    ...
    if (saveType) {
        ...
        let newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/DI/1`
        if (lastObject) {
            newDocId = `${branchObj.branchCode}${getYearShortCode(new Date())}/DI/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`
        }
        return newDocId
    }
